    ...
    class ModuleLoader {
        ...
        define(name, deps, factory, lazy = false) {
            ...
            if (!Array.isArray(deps)) {
                throw new Error(`Module dependencies should be a list of strings, got: ${String(deps)}`);
            }
            ...
            const findCycle = (currentModuleNames, visited) => {
                for (const name of currentModuleNames || []) {
                    if (visited.has(name)) {
                        ...
                        return cycleModuleNames.slice(cycleModuleNames.indexOf(name)).map((j) => `"${j}"`).join(" => ");
                    }
...
odoo.define('@web/legacy/js/public/minimal_dom', ['@web/core/utils/ui'], function(require) {
    ...
    __exports.makeAsyncHandler = makeAsyncHandler;

    function makeAsyncHandler(fct, preventDefault, stopPropagation, stopImmediatePropagation) {
        let pending = false;
    ...
    __exports.makeButtonHandler = makeButtonHandler;

    function makeButtonHandler(fct, preventDefault, stopPropagation, stopImmediatePropagation) {
...
odoo.define('@web/legacy/js/public/lazyloader', ['@web/legacy/js/public/minimal_dom'], function(require) {
    ...
    function waitLazy() {
        ...
        const loadingEffectButtonEls = [...mainEl.querySelectorAll(BUTTON_HANDLER_SELECTOR)].filter(el => {
            return !el.classList.contains('o_no_wait_lazy_js') && !(el.nodeName === 'A' && el.href && el.getAttribute('href') !== '#');
        });
        ...
        for (const buttonEl of loadingEffectButtonEls) {
            for (const eventType of loadingEffectEventTypes) {
                const loadingEffectHandler = eventType === 'click' ? makeButtonHandler(waitForLazyAndRetrigger, true, true, true) : makeAsyncHandler(waitForLazyAndRetrigger, true, true, true);
...
odoo.define('@website/utils/misc', ['@web/core/browser/cookie'], function(require) {
    ...
    __exports.setUtmsHtmlDataset = setUtmsHtmlDataset;

    function setUtmsHtmlDataset() {
    ...
    __exports.verifyHttpsUrl = verifyHttpsUrl;

    function verifyHttpsUrl(link) {
...
odoo.define('@website/js/content/auto_hide_menu', [], function(require) {
    ...
    __exports.setupIgnoreDOMMutations = setupIgnoreDOMMutations;

    function setupIgnoreDOMMutations(fn) {
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        if (header) {
            ...
            if (!topMenu.querySelector(`:scope > :not(${unfoldable})`) || header.classList.contains("o_no_autohide_menu")) {
